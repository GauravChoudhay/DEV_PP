...
function solveQuestion(qLink) {
  return new Promise(function (resolve, reject) {
    ...
    gotoPromise
      ...
      .then(function(){
        //wating to get new dom on editoral
        let waitFor5SecPromise = tab.mainFrame().waitForTimeout(5000);
        return waitFor5SecPromise;
      })
      .then(function () {
        //making array using $$() to check wheather there solution is lock or unlock
        let isunlockButtonPromise = tab.$$(".ui-btn.ui-btn-normal.ui-btn-primary.ui-btn-styled");
        
        return isunlockButtonPromise;
      })
      .then(function(isUnlockButton){
        //console.log(isUnlockButton.length);
        // let unlockButtonPromise;
        if(isUnlockButton.length!=0){
          let unlockButtonPromise = tab.click(".ui-btn.ui-btn-normal.ui-btn-primary.ui-btn-styled");
          return unlockButtonPromise;
        }
      })
      .then(function () {
